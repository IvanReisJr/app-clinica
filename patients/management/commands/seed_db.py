...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # 1. Limpar dados para evitar conflitos
        ...
        for i in range(5):
            ...
            Appointment.objects.get_or_create(
                ...
                appointment_time=timezone.now().replace(hour=random.randint(8, 17), minute=0, second=0, microsecond=0).time(),
                defaults={'status': 'atendido'}
            )
        ...
        for i, hora in enumerate(horas_hoje):
            ...
            Appointment.objects.create(
                ...
                appointment_time=timezone.now().replace(hour=hora, minute=0, second=0, microsecond=0).time(),
                status=status,
        ...
        past_apps = Appointment.objects.filter(status='atendido').order_by('-appointment_date')[:5]
        for i, app in enumerate(past_apps):
            # Prontuário com Atestado (apenas em alguns)
            atestado_dias = 3 if i % 2 == 0 else None
            MedicalRecord.objects.get_or_create(
                ...
                defaults={
                    ...
                    'atestado_dias': atestado_dias,
                    'atestado_inicio': app.appointment_date if atestado_dias else None,
                    'atestado_retorno': (app.appointment_date + timedelta(days=3)) if atestado_dias else None,
                }
            )
            
            # Criar 2 prescrições para cada atendimento histórico
            Prescription.objects.get_or_create(
                patient=app.patient,
                professional=app.professional,
                appointment=app,
                medication='Amoxicilina 500mg' if i % 2 == 0 else 'Ibuprofeno 600mg',
                defaults={
                    'dose': '1 cápsula',
                    'frequency': '8/8h',
                    'duration': '7 dias',
